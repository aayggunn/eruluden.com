...
if ($alici_bilgi_sorgu->num_rows > 0) {
    ...
    if (isset($_POST['mesaj_gonder'])) {
        ...
        if ($mesaj_kaydet->execute()) {
            ...
            $alici_email_sorgu = $baglanti->prepare("SELECT email FROM eruluden_uyeler WHERE uye_id = ?");
            if ($alici_email_sorgu === false) {
                die('Sorgu hazırlama hatası: ' . $baglanti->error);
            }

            $alici_email_sorgu->bind_param("i", $alici_id);
            $alici_email_sorgu->execute();
            $alici_email_sorgu->bind_result($alici_email);
            $alici_email_sorgu->fetch();
            $alici_email_sorgu->close();

            if (!empty($alici_email)) {
                $konu = "Yeni mesajınız var!";
                $mesaj_icerigi = "Merhaba " . $alici_ad . ",\n\n" . "Yeni bir mesajınız var:\n\n" . $mesaj . "\n\n" . "<a href='https://www.eruluden.com/login.php'>Mesajı görmek için siteye giriş yapın.</a>";
                ...
                echo "Alıcının e-posta adresi bulunamadı.";
            }
        } else {
...
?>
					...
					<div class="gonder">
						<button class="send" type="submit" name="mesaj_gonder"><i class="fa-solid fa-paper-plane"></i> </button>
					</div>
						</form>
	</div>
 ...
 <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            form.addEventListener("submit", function(event) {
                ...
                var mesaj = mesajInput.value.trim(); // Mesajı al ve boşlukları temizle

                if (mesaj !== "") {
                    // Mesaj varsa AJAX ile gönder
                    var xhr = new XMLHttpRequest();
                    xhr.open("POST", "mesaj_gonder.php", true);
                    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

                    xhr.onreadystatechange = function() {
                        if (xhr.readyState === XMLHttpRequest.DONE) {
                            if (xhr.status === 200) {
                                // Başarılı gönderim durumunda mesajları güncelle
                                document.getElementById("chat-box").innerHTML = xhr.responseText;
                                mesajInput.value = ""; // Formu temizle
                            } else {
                                console.error("Mesaj gönderme hatası: " + xhr.status);
                            }
                        }
                    };

                    xhr.send("mesaj=" + encodeURIComponent(mesaj));
                }
            });

            // Enter tuşuna basıldığında formu gönder
            mesajInput.addEventListener("keypress", function(event) {
                if (event.key === "Enter") {
                    form.dispatchEvent(new Event("submit"));
                    event.preventDefault(); // Enter tuşunun normal işlevini engelle
                }
            });
        });
    </script>
